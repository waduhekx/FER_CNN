...
def deepnn(x,y_):
  ...
  h_conv3_flat = tf.reshape(norm2, [-1, 12 * 12 * 64])
...
def train_model(train_data):
  ...
  W_fc2 = weight_variables([384, 192])
  b_fc2 = bias_variable([192])
  h_fc2 = tf.add(tf.matmul(h_fc1_drop, W_fc2), b_fc2)
  ...
  with tf.Session() as sess:
    ...
    for step in range(max_train_steps):
      ...
      '''
      image, conv1, pool1, norm11, conv2, pool2, norm22, flat = sess.run((x_image, h_conv1, h_pool1, norm1,
                                                                          h_conv2, h_pool2, norm2, h_conv3_flat),
                                                                         feed_dict={
                                                                           x:batch[0], y_:batch[1]
                                                                         })
      print('image_shape = ', image.shape, ' conv1_shape = ', conv1.shape,
            '  h_pool1 = ', pool1.shape, '  norm1.shape = ', norm11.shape,
            ' h_conv2 = ', conv2.shape, '  h_pool2 = ', pool2.shape,
            ' norm2 = ', norm22.shape, 'h_conv3_flat', flat.shape)
      '''
      ...
      if step % 1000 == 0:
        print('*Test accuracy %g' % accuracy.eval(feed_dict={
          x: fer2013.validation.images, y_: fer2013.validation.labels, keep_prob:1}))
